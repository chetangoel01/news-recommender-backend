#!/usr/bin/env python3
"""
Final test for view tracking and duplicate prevention system.
"""
...
def test_view_tracking_basic():
    ...
    try:
        ...
        test_user = User(
            username=f"test_user_view_{unique_id}",
            email=f"test_view_{unique_id}@example.com",
        ...
        test_article = Article(
            title=f"Test Article for View Tracking {unique_id}",
            summary=f"This is a test article for view tracking {unique_id}",
            content=f"Full content of test article {unique_id}",
...
def test_skip_tracking():
    ...
    try:
        ...
        unique_id = str(uuid.uuid4())[:8]
        test_user = User(
            username=f"test_user_skip_{unique_id}",
            email=f"test_skip_{unique_id}@example.com",
            password_hash="hashed_password",
            created_at=datetime.now(timezone.utc)
        )
        session.add(test_user)
        ...
        # Create a test article
        test_article = Article(
            title=f"Test Article for Skip Tracking {unique_id}",
            summary=f"This is a test article for skip tracking {unique_id}",
            content=f"Full content of test article {unique_id}",
            url=f"https://example.com/test-skip-article-{unique_id}",
            source_name="TestSource",
            category="business",
            published_at=datetime.now(timezone.utc),
            views=50,
...
def test_recommendations_exclude_viewed():
    ...
    try:
        ...
        unique_id = str(uuid.uuid4())[:8]
        test_user = User(
            username=f"test_user_rec_{unique_id}",
            email=f"test_rec_{unique_id}@example.com",
            password_hash="hashed_password",
            created_at=datetime.now(timezone.utc)
        )
        session.add(test_user)
        ...
        for i in range(5):
            article = Article(
                title=f"Test Article {i+1} {unique_id}",
                summary=f"This is test article {i+1} {unique_id}",
                content=f"Full content of test article {i+1} {unique_id}",
                url=f"https://example.com/test-article-{i+1}-{unique_id}",
                source_name="TestSource",
                category="technology",
...
def run_all_tests():
    ...
    if passed == total:
        ...
        print("✅ View tracking prevents duplicate articles in feed")
    else:
